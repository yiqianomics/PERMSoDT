...
.build_hex_centers <- function(xmin, xmax, ymin, ymax, bin_size = 0.02){
  width  <- xmax - xmin
  xbins  <- max(10L, round(width / bin_size))
  # dense uniform points are cheap and ensure grid coverage
  n_dummy <- max(5000L, xbins * xbins)       # heuristic; cheap but robust
  xs <- runif(n_dummy, xmin, xmax)
  ys <- runif(n_dummy, ymin, ymax)
  hb <- hexbin(xs, ys, xbins = xbins, xbnds = c(xmin, xmax), ybnds = c(ymin, ymax))
  cc <- hcell2xy(hb)
  centers <- cbind(cc$x, cc$y)               # n_cell x 2
  colnames(centers) <- c("cx","cy")
  centers
}

## -- Turn sample geometry into OTU probabilities over fixed centers --
# Abundant [0,0.2r], Moderate [0.4,0.8r], Rare [0.8,1.0r]; density ~ (d/r)^alpha with band multipliers.
.center_probs <- function(centers, o, r, alpha, k_abund=1, k_mod=1, k_rare=1){
  dx <- centers[,1] - o[1]; dy <- centers[,2] - o[2]
  d  <- sqrt(dx*dx + dy*dy)
  in_ball <- (d <= r)
  if(!any(in_ball)){
    # fallback: ensure at least nearest cell gets mass
    j <- which.min(d); p <- rep(0, nrow(centers)); p[j] <- 1
    return(p)
  }
  x <- pmin(1, pmax(0, d / r))               # scaled radius in [0,1]
  base <- x^alpha
  band <- rep(1, length(d))
  band[x <= 0.20]        <- k_abund
  ...
  s <- sum(prob)
  if(s <= 0){
    j <- which.min(d); prob[j] <- 1; s <- 1
  }
  prob / s
}

## -- MAIN: fixed-grid simulator; can optionally build a midpoint-rooted tree --
ConMiseq_fixedGrid <- function(
    n = 40,                # samples
    N_pts = 300,           # reads per sample
    # group/confounding
    Delta = 0.10,          # baseline centroid distance between groups
    bo = 0.35,             # confounder-induced centroid shift (x-direction)
    ps0 = 1.8,             # slope in ps = plogis(-0.3 + ps0 * Z)
    # geometry channels
    r0 = 0.15, beta_r = 0, delta_r = 0.06, eta_r = 0,
    a0 = 0.5,  beta_a = 0, delta_a = 0.60, eta_a = 0,
    k0 = 0,    beta_k = 0, delta_k = 0.0,  eta_k = 0,
    # grid controls
    bin_size = 0.02,       # controls number of OTUs (hex density)
    bbox_margin = 0.10,    # extra margin (absolute units) around sample envelopes
    # filtering (speeds up downstream distances/tree)
    min_prev = 0.05,       # drop OTUs present in <5% samples (or integer threshold)
    min_var  = 1e-8,       # drop near-constant OTUs
    # tree
    build_tree = TRUE,     # set FALSE if you do not need UniFrac
    seed = 1
){
  set.seed(seed)
  
  ## 1) Confounder and group
  Z  <- rnorm(n)
  ps <- plogis(-0.3 + ps0*Z)
  G  <- rbinom(n, 1, ps)
  
  ## 2) Sample-specific geometry
  o0 <- c(-Delta/2, 0); o1 <- c( Delta/2, 0)
  o_list <- lapply(seq_len(n), function(i){
    base <- if(G[i]==0) o0 else o1
    base + c(bo*Z[i], 0)
  })
  r  <- pmax(0.05, r0 + beta_r*G + delta_r*Z + eta_r*G*Z)
  a  <- a0 + beta_a*G + delta_a*Z + eta_a*G*Z
  kR <- exp(k0 + beta_k*G + delta_k*Z + eta_k*G*Z)
  kM <- 1.0; kA <- 1.0
  
  ## 3) Build a single global hex grid covering all sample disks
  ...
  centers <- .build_hex_centers(xs_min, xs_max, ys_min, ys_max, bin_size = bin_size)
  K <- nrow(centers)
  
  ## 4) For each sample, compute OTU probabilities over centers → multinomial counts
  ...
  ## 5) Cheap OTU filters (optional but fast)
  keep <- colSums(OTU) > 0
  ...
  if(!build_tree){
    return(phyloseq(ps_otu, ps_sd))
  }
  
  # ---- Build a midpoint-rooted NJ tree from OTU relative profiles (fast & reproducible) ----
  otu_rel <- sweep(OTU, 1, pmax(rowSums(OTU), 1), "/")
  D_otu   <- dist(t(otu_rel))               # K' x K'
  tr      <- ape::nj(D_otu)
  tr$tip.label <- colnames(otu_rel)
  tr      <- phangorn::midpoint(tr)
  
  phyloseq(ps_otu, ps_sd, phy_tree(tr))
}
...
Ps_levels <- c(0.2, 2)  # weak/strong confounding
...
analyze_one <- function(D, meta) {
  ...
  t1 <- try(vegan::adonis2(D ~ G, data = meta, permutations = 999), silent = TRUE)
  F1 <- P1 <- NA_real_
  ...
  # Pooled (include Z)
  t2 <- try(vegan::adonis2(D ~ G + Z, data = meta, permutations = 999), silent = TRUE)
  F2 <- P2 <- NA_real_
  ...
  # IPW (deconfounded)
  t3 <- try(ipw_permanova(as_square(D), meta$G, meta$Z), silent = TRUE)
  F3 <- P3 <- NA_real_
...
for (n in Ns) {
  for (Delta in Deltas) {
    for (ps in Ps_levels) {
      ...
      psA <- ConMiseq_fixedGrid(
        n = n, N_pts = 400,
        Delta = Delta,      # true group separation
        bo = 0.25,          # Z shifts centroids -> apparent separation
        ps = ps,            # confounding strength
        r0 = 0.15, beta_r = 0, delta_r = 0,  eta_r = 0,
        a0 = 0.5,  beta_a = 0, delta_a = 0,  eta_a = 0,
        k0 = 0,    beta_k = 0, delta_k = 0,  eta_k = 0,
        bin_size = 0.015,
        seed = sim_seed     # seed varies within replicate
      )
