...
gg1 <- ggplot(plot_df, aes(x = Delta, y = prop_sig,
                          color = method, linetype = method, shape = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
...
plot_df <- by_panel %>%
  mutate(
    method = factor(recode(method, !!!method_labels),
                    levels = c("adonis2 (unadjusted)", "adonis2 (+ Z)", "IPW-PERMANOVA")),
    n_f  = factor(n,  levels = c(40, 60, 80), labels = c("n = 40", "n = 60", "n = 80")),
    ps_f = factor(ps, levels = c(0.1, 0.25),   labels = c("ps = 0.1 (weak)", "ps = 0.25 (strong)"))
  )

gg2 <- ggplot(plot_df, aes(x = Delta, y = prop_sig,
                          color = method, linetype = method, shape = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_y_continuous(limits = c(0, 1),
...
plot_df <- by_panel %>%
  mutate(
    method = factor(recode(method, !!!method_labels),
                    levels = c("adonis2 (unadjusted)", "adonis2 (+ Z)", "IPW-PERMANOVA")),
    n_f  = factor(n,  levels = c(40, 60, 80), labels = c("n = 40", "n = 60", "n = 80")),
    ps_f = factor(ps, levels = c(0.2, 1.2),   labels = c("ps = 0.2 (weak)", "ps = 1.2 (strong)"))
  )

gg3 <- ggplot(plot_df, aes(x = Delta, y = prop_sig,
                          color = method, linetype = method, shape = method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_y_continuous(limits = c(0, 1),
...
# Helper function to drop title and legend
...
# Keep legend in the first plot only
gg1_clean <- strip_plot(gg1)
gg2_clean <- strip_plot(gg2)
gg3_clean <- strip_plot(gg3) +
              theme(legend.position = "bottom")

# Combine them in one row (or adjust layout)
combined <- (gg1_clean / gg2_clean / gg3_clean) +
...
ggsave("SimulatedResult.png", combined,
       width = 8, height = 12, dpi = 1000)
