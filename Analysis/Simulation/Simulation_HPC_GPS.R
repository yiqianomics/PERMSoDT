...
setwd("/home/yxz3116/IPWPERMANOVA/Continuous")
################################ Simulation ########################################
...
# evaluate kernel density at observed points (for f_X); simple linear interp
.density_at <- function(x, at){
  kd <- density(x, n = max(512, length(x)*2))
  approx(kd$x, kd$y, xout = at, rule = 2)$y
}

## -------------------------- MAIN FUNCTION ---------------------------

ConMiseq_fixedGrid_GPS <- function(
    n = 40,                 # samples
    N_pts = 300,            # reads per sample
    # exposure model X|Z (choose family and parameters)
    x_family = c("normal","beta01","gamma+"),
    x_coef   = c(beta0 = 0, beta1 = 1.5),  # mean link: mu(Z) = beta0 + beta1*Z (then mapped per family)
    x_scale  = 0.7,         # normal: sigma; beta: precision phi; gamma: shape k (see below)
    # mapping mu(Z): for beta, pass through plogis to (0,1); for gamma, exp(mu) for mean>0
    # confounder Z
    Z_sd = 1.0,             # Z ~ N(0, Z_sd^2)
    # geometry: how X (and optionally Z) drives community
    # r = radius (richness), a = alpha (evenness), kR = rare-band scaling
    r0 = 0.15,  ar = c(bx = 0.06, bz = 0.00),   # r = r0 + bx*X + bz*Z
    a0 = 0.5,   aa = c(bx = 0.80, bz = 0.00),   # a = a0 + bx*X + bz*Z
    k0 = 0.0,   ak = c(bx = 0.00, bz = 0.00),   # log kR = k0 + bx*X + bz*Z
    # optional centroid baseline separation by X (set to 0 if not needed)
    Delta_x = 0.00,                             # baseline group-like separation via X (rarely needed)
    bo_z    = 0.00,                             # extra centroid shift along x by Z (if you also want Z->membership)
    # grid controls
    centers = NULL, bin_size = 0.02, bbox_margin = 0.10, target_K = NULL,
    min_prev = 0.05, min_var = 1e-8,
    # GPS weights
    w_trim_q = 0.99,        # quantile for trimming stabilized GPS weights
    # tree
    build_tree = TRUE,
    # reproducibility
    seed = 1,
  ...
  set.seed(seed)
  x_family <- match.arg(x_family)
  
  ## 1) Confounder
  Z <- rnorm(n, sd = Z_sd)
  
  ## 2) Exposure X | Z  (GPS model components)
  mu_lin <- x_coef["beta0"] + x_coef["beta1"] * Z
  
  if (x_family == "normal"){
    # X | Z ~ Normal(mu, sigma^2), GPS = dnorm(X; mu, sigma)
    sigma <- x_scale
    X <- rnorm(n, mean = mu_lin, sd = sigma)
    gps_cond <- dnorm(X, mean = mu_lin, sd = sigma)
  } else if (x_family == "beta01"){
    # X in (0,1). Mean m = plogis(mu_lin), precision phi = x_scale
    m <- plogis(mu_lin); phi <- x_scale
    a <- pmax(1e-6, m * phi); b <- pmax(1e-6, (1-m) * phi)
    X <- rbeta(n, a, b)
    gps_cond <- dbeta(X, a, b)
  } else { # "gamma+"
    # X > 0. Mean m = exp(mu_lin). Use shape k = x_scale, rate = k/m
    m <- exp(mu_lin); k <- x_scale; rate <- k / m
    X <- rgamma(n, shape = k, rate = rate)
    gps_cond <- dgamma(X, shape = k, rate = rate)
  }
  # Marginal density f_X(X): kernel density estimate
  fX <- .density_at(X, X)
  w_gps_stab <- pmax(1e-12, fX) / pmax(1e-12, gps_cond)
  w_gps_trim <- pmin(w_gps_stab, quantile(w_gps_stab, w_trim_q, names = FALSE))
  
  ## 3) Sample-specific geometry driven by X (and optional Z)
  # Centroids: optional X-driven baseline separation + Z membership shift
  o0 <- c(-Delta_x/2, 0); o1 <- c( Delta_x/2, 0)
  # Here we don't have binary groups; choose side by sign(X - median(X)) only if Delta_x != 0
  side <- if (Delta_x == 0) rep(0L, n) else as.integer(X > median(X))
  o_list <- lapply(seq_len(n), function(i){
    base <- if(side[i]==0) o0 else o1
    base + c(bo_z * Z[i], 0)
  })
  # Radius / evenness / rare-band as functions of X (and Z)
  r  <- pmax(0.05, r0 + ar["bx"]*X + ar["bz"]*Z)
  a  <- a0 + aa["bx"]*X + aa["bz"]*Z
  kR <- exp(k0 + ak["bx"]*X + ak["bz"]*Z)
  kM <- 1.0; kA <- 1.0
  
  ## 4) Build/use grid, then generate counts by multinomial per sample
  # BBox from centroids and radii
  ...
  colnames(OTU) <- paste0("OTU_", seq_len(K)); rownames(OTU) <- paste0("S", seq_len(n))
  
  ## 5) Filtering
  keep <- colSums(OTU) > 0
  OTU  <- OTU[, keep, drop=FALSE]; centers <- centers[keep,,drop=FALSE]
  if(!is.null(min_prev)){
  ...
  ## 6) phyloseq (tree optional)
  ps_otu <- otu_table(OTU, taxa_are_rows = FALSE)
  ps_sd  <- sample_data(data.frame(
    SampleID = rownames(OTU),
    X = X, Z = Z,
    gps = pmax(1e-12, gps_cond),
    fX  = pmax(1e-12, fX),
    w_gps_stab = w_gps_stab,
    w_gps_trim = w_gps_trim,
...
Ps_levels <- c(0.1, 0.25)  # weak/strong confounding
...
analyze_one <- function(D, meta) {
  ...
  t1 <- try(vegan::adonis2(D ~ X, data = meta, permutations = 999), silent = TRUE)
  F1 <- P1 <- NA_real_
  ...
  # Pooled (include Z)
  t2 <- try(vegan::adonis2(D ~ X + Z, data = meta, permutations = 999), silent = TRUE)
  F2 <- P2 <- NA_real_
  ...
  # IPW (deconfounded)
  t3 <- try(ipw_permanova(as_square(D), meta$X, meta$Z), silent = TRUE)
  F3 <- P3 <- NA_real_
...
for (n in Ns) {
  for (Delta in Deltas) {
    for (ps in Ps_levels) {
      ...
      psA <- ConMiseq_fixedGrid_GPS(
        n=n, N_pts=400,
        x_family="normal", x_coef=c(beta0=0, beta1=ps), x_scale=0.5,  
        ar=c(bx=0, bz=0.2),       # r as a function of X or Z
        aa=c(bx=0, bz=0.2),       # a as a function of X or Z
        ak=c(bx=0, bz=0.2),       # rare-band as a function of X or Z
        bo_z=0.5, Delta_x=Delta,  
        seed = sim_seed
      )
